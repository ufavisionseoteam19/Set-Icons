...
function run_phase2(string $docroot): void {
    ...
    // หา menu_id ที่ผูกกับ location มือถือ (เพื่อกำหนด size 18)
    $locs = get_nav_menu_locations();
    $mobileMenuIds = [];
    foreach ($locs as $loc => $mid) {
        if (in_array($loc, $MOBILE_LOCATIONS, true)) $mobileMenuIds[] = (int)$mid;
    }

    // วน "ทุกเมนู" ในระบบ (ครอบคลุม Top Menu / เมนูที่ผูกผ่าน Blocksy header element)
    $menus = wp_get_nav_menus();
    if (empty($menus)) { echo "  ไม่พบเมนูในระบบ\n"; exit(0); }
